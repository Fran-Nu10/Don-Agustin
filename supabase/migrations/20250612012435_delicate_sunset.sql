/*
  # Actualizar constraint de estados en tabla clients

  1. Cambios en constraint
    - Actualizar datos existentes para cumplir con nuevos estados
    - Eliminar constraint anterior
    - Crear nuevo constraint con estados actualizados
  
  2. Migración de datos
    - Migrar bookings a clients
    - Agregar clientes de ejemplo con nuevos estados
  
  3. Estados permitidos
    - 'nuevo' - Cliente recién llegado
    - 'presupuesto_enviado' - Presupuesto enviado
    - 'en_seguimiento' - En proceso de seguimiento
    - 'cliente_cerrado' - Proceso completado
*/

-- Primero, actualizar todos los registros existentes para que cumplan con el nuevo constraint
UPDATE clients 
SET status = CASE 
  WHEN status = 'en_proceso' THEN 'en_seguimiento'
  WHEN status = 'cerrado' THEN 'cliente_cerrado'
  WHEN status NOT IN ('nuevo', 'presupuesto_enviado', 'en_seguimiento', 'cliente_cerrado') THEN 'nuevo'
  ELSE status
END;

-- Ahora eliminar el constraint existente

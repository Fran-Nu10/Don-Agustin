/*
  # Integrar sistema de reservas con CRM

  1. Cambios en tabla clients
    - Actualizar constraint de status con nuevos valores
    - Migrar datos de bookings existentes a clients
    - Actualizar datos existentes para cumplir con nuevo constraint

  2. Migración de datos
    - Convertir bookings en clientes del CRM
    - Preservar información de viajes en el mensaje
    - Asignar estados apropiados

  3. Datos de ejemplo
    - Agregar clientes con diferentes estados
    - Mostrar flujo completo del CRM
*/

-- Primero, actualizar todos los registros existentes que no cumplan con el nuevo constraint
UPDATE clients 
SET status = CASE 
  ...
  ELSE 'nuevo'
END
WHERE status NOT IN ('nuevo', 'presupuesto_enviado', 'en_seguimiento', 'cliente_cerrado');

-- Ahora actualizar el constraint de status en clients
